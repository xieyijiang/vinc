...
const templatesDir = path.join(__dirname, '..', 'templates/store') // templates文件夹的路径
...
// 创建目录和文件
const directoryParts = storeName.split('/');
const directory = path.join(__dirname, '..', 'src', 'stores', ...directoryParts.slice(0, -1));
...
if (fs.existsSync(filePath)) {
  ...
  // 替换模板中的占位符
  defaultCode = defaultCode.replace(/_storeName/g, storeName);
